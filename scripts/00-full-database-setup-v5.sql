...
-- Step 5: Create the report table (media_urls re-added)
CREATE TABLE report (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  chadabaz_id UUID REFERENCES chadabaz(id) ON DELETE CASCADE,
  report_text TEXT NOT NULL,
  media_urls TEXT[] DEFAULT '{}', -- Re-added media_urls
  submitted_by TEXT,
...
CREATE POLICY "Public can view approved chadabaz profiles" ON chadabaz
  FOR SELECT USING (approved_status = TRUE);
...
SELECT
  ...
  c.name,
  r.report_text,
  r.media_urls, -- Include media_urls in the view
  r.submitted_by,
  r.submitted_at
FROM report r
...
BEGIN
    ...
    INSERT INTO chadabaz (name, area, political_party_id, facebook_link, profile_picture, description, approved_status) VALUES
    ('মোহাম্মদ করিম', 'ঢাকা, ধানমন্ডি', party_awami_league_id, 'https://facebook.com/mohammad.karim', '/placeholder.svg?height=100&width=100', 'একজন পরিচিত চাঁদাবাজ, বিভিন্ন ব্যবসায়ীদের কাছ থেকে নিয়মিত চাঁদা আদায় করে।', TRUE),
    ('আব্দুল রহিম', 'চট্টগ্রাম, আগ্রাবাদ', party_bnp_id, 'https://facebook.com/abdul.rahim', '/placeholder.svg?height=100&width=100', 'রাজনৈতিক প্রভাব খাটিয়ে চাঁদাবাজি করে।', TRUE),
    ('সালাহউদ্দিন আহমেদ', 'সিলেট, জিন্দাবাজার', party_jatiya_id, NULL, '/placeholder.svg?height=100&width=100', 'ছোট ব্যবসায়ীদের টার্গেট করে।', TRUE),
    ('রফিকুল ইসলাম', 'রাজশাহী, বোয়ালিয়া', party_awami_league_id, NULL, '/placeholder.svg?height=100&width=100', 'এলাকার রিকশাওয়ালাদের কাছ থেকে চাঁদা নেয়।', TRUE),
    ('নাসির উদ্দিন', 'খুলনা, দৌলতপুর', party_bnp_id, NULL, '/placeholder.svg?height=100&width=100', 'নতুন এলাকায় এসে চাঁদাবাজি শুরু করেছে।', TRUE),
    ('আলী হাসান', 'বরিশাল, বন্দর', party_jamaat_id, NULL, '/placeholder.svg?height=100&width=100', 'তদন্তাধীন চাঁদাবাজ।', FALSE),
    ('কামাল উদ্দিন', 'রংপুর, গঙ্গাচড়া', party_awami_league_id, NULL, '/placeholder.svg?height=100&width=100', 'স্থানীয় এলাকায় চাঁদাবাজির অভিযোগ পাওয়া গেছে।', FALSE),
    ('জাহিদ হোসেন', 'ময়মনসিংহ, ত্রিশাল', party_bnp_id, NULL, '/placeholder.svg?height=100&width=100', 'নতুন অভিযোগ এসেছে।', FALSE)
    ...
    INSERT INTO report (chadabaz_id, report_text, status, submitted_at, media_urls) VALUES
    (karim_id, 'এই ব্যক্তি আমাদের এলাকায় নিয়মিত চাঁদা দাবি করে। দোকানদারদের কাছ থেকে জোর করে টাকা নেয়। স্থানীয় যুবকদের দিয়ে ভয় দেখায়। গত মাসে আমার দোকানে এসে ৫০০০ টাকা চাঁদা দাবি করেছে।', 'approved', NOW() - INTERVAL '15 days', ARRAY['/placeholder.svg?height=200&width=300']),
    (karim_id, 'গত সপ্তাহে আমার দোকানে এসে ৫০০০ টাকা চাঁদা দাবি করেছে। না দিলে দোকান বন্ধ করে দেওয়ার হুমকি দিয়েছে।', 'approved', NOW() - INTERVAL '12 days', ARRAY['/placeholder.svg?height=200&width=300']),
    (karim_id, 'এলাকার সব দোকানদারদের কাছ থেকে মাসিক চাঁদা নেয়। পুলিশে অভিযোগ করলেও কোনো ব্যবস্থা নেওয়া হয়নি।', 'approved', NOW() - INTERVAL '10 days', ARRAY['/placeholder.svg?height=200&width=300']),
    (karim_id, 'নতুন ব্যবসায়ীদের কাছ থেকে বেশি টাকা দাবি করে। হুমকি দিয়ে টাকা আদায় করে।', 'approved', NOW() - INTERVAL '8 days', ARRAY['/placeholder.svg?height=200&width=300']),
    (karim_id, 'স্থানীয় বাজারে তার দাপট। কেউ কিছু বলতে পারে না ভয়ে।', 'approved', NOW() - INTERVAL '5 days', ARRAY['/placeholder.svg?height=200&width=300'])
    ON CONFLICT DO NOTHING;

    -- Insert reports for Rafiqul Islam (4 reports)
    INSERT INTO report (chadabaz_id, report_text, status, submitted_at, media_urls) VALUES
    (rafiq_id, 'এলাকার ছোট ব্যবসায়ীদের কাছ থেকে সাপ্তাহিক চাঁদা নেয়। না দিলে দোকান ভাঙচুরের হুমকি দেয়।', 'approved', NOW() - INTERVAL '20 days', ARRAY['/placeholder.svg?height=200&width=300']),
    (rafiq_id, 'রিকশাওয়ালাদের কাছ থেকেও চাঁদা নেয়। দৈনিক ২০ টাকা করে।', 'approved', NOW() - INTERVAL '18 days', ARRAY['/placeholder.svg?height=200&width=300']),
    (rafiq_id, 'স্থানীয় মার্কেটে তার লোকজন পাহারা দেয়। সবাই ভয়ে থাকে।', 'approved', NOW() - INTERVAL '15 days', ARRAY['/placeholder.svg?height=200&width=300']),
    (rafiq_id, 'নতুন দোকান খুললেই চাঁদা দাবি করে। অনেকেই ব্যবসা বন্ধ করে দিয়েছে।', 'approved', NOW() - INTERVAL '12 days', ARRAY['/placeholder.svg?height=200&width=300'])
    ON CONFLICT DO NOTHING;

    -- Insert reports for Abdul Rahim (3 reports)
    INSERT INTO report (chadabaz_id, report_text, status, submitted_at, media_urls) VALUES
    (rahim_id, 'স্থানীয় ব্যবসায়ীদের কাছ থেকে নিয়মিত চাঁদা আদায় করে। হুমকি দিয়ে টাকা আদায় করে।', 'approved', NOW() - INTERVAL '25 days', ARRAY['/placeholder.svg?height=200&width=300']),
    (rahim_id, 'পুলিশে অভিযোগ করলেও কোনো ব্যবস্থা নেওয়া হয়নি। রাজনৈতিক প্রভাব আছে।', 'approved', NOW() - INTERVAL '22 days', ARRAY['/placeholder.svg?height=200&width=300']),
    (rahim_id, 'এলাকার মানুষ তাকে ভয় পায়। কেউ প্রতিবাদ করতে পারে না।', 'approved', NOW() - INTERVAL '20 days', ARRAY['/placeholder.svg?height=200&width=300'])
    ON CONFLICT DO NOTHING;

    -- Insert reports for Salahuddin Ahmed (2 reports)
    INSERT INTO report (chadabaz_id, report_text, status, submitted_at, media_urls) VALUES
    (salah_id, 'ছোট ব্যবসায়ীদের কাছ থেকে চাঁদা নেয়। মাসিক ১০০০ টাকা করে।', 'approved', NOW() - INTERVAL '30 days', ARRAY['/placeholder.svg?height=200&width=300']),
    (salah_id, 'স্থানীয় বাজারে তার নিয়ন্ত্রণ। সবাই বাধ্য হয়ে টাকা দেয়।', 'approved', NOW() - INTERVAL '28 days', ARRAY['/placeholder.svg?height=200&width=300'])
    ON CONFLICT DO NOTHING;

    -- Insert reports for Nasir Uddin (1 report)
    INSERT INTO report (chadabaz_id, report_text, status, submitted_at, media_urls) VALUES
    (nasir_id, 'নতুন এলাকায় এসে চাঁদাবাজি শুরু করেছে। দোকানদাররা চিন্তিত।', 'approved', NOW() - INTERVAL '35 days', ARRAY['/placeholder.svg?height=200&width=300'])
    ON CONFLICT DO NOTHING;

    -- Insert some pending reports (for chadabaz that are not yet approved)
    INSERT INTO report (chadabaz_id, report_text, status, submitted_at, media_urls) VALUES
    (ali_id, 'এই ব্যক্তি সম্পর্কে তদন্ত চলছে। শীঘ্রই আরো তথ্য পাওয়া যাবে।', 'pending', NOW() - INTERVAL '3 days', ARRAY['/placeholder.svg?height=200&width=300']),
    (kamal_id, 'স্থানীয় এলাকায় চাঁদাবাজির অভিযোগ পাওয়া গেছে।', 'pending', NOW() - INTERVAL '2 days', ARRAY['/placeholder.svg?height=200&width=300']),
    (jahid_id, 'নতুন অভিযোগ এসেছে। যাচাই করা হচ্ছে।', 'pending', NOW() - INTERVAL '1 day', ARRAY['/placeholder.svg?height=200&width=300'])
    ON CONFLICT DO NOTHING;
